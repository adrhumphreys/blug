...
_At this point we've gone back to the client with assets that work for the page in question so their marketing team can launch the campaign while we investigate the issue further._
...
First we start with the `onBeforeWrite` we'll edit the Extensible class from the framework to log the extensions applied to the class and the methods being called. We can see that it gets through _most_ of the `onBeforeWrite` calls but hangs at the Files `onBeforeWrite`. Alright lets dig into that method.
